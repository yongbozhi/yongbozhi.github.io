...
![img](java基础/clip_image002.jpg)

![img](java基础/clip_image004.gif)
...
很明显, 接口其中一个存在意义就是为了实现多态.：农夫喂不同动物水
...
在动物的派生类（老虎需要，山羊不需要）中, 有些类需要, 有些不需要. 
...
![Person p.new person() ;  class clazz.p.getclass(); ](java基础/clip_image008.jpg)

调用某个类的 class 属性来获取该类对应的 Class 对象 

![ssel) ](java基础/clip_image010.jpg)
...
- - **序列化：将对象写入到****IO****流中**
  ...
  - **使用场景：所有可在网络上传输的对象都必须是可序列化的，**比如RMI（remote method invoke,即远程方法调用），传入的参数或返回的对象都是可序列化的，否则会出错；**所有需要保存到磁盘的****java****对象都必须是可序列化的。通常建议：程序创建的每个****JavaBean****类都实现****Serializeable****接口。**
...
如果是 String s =“xxx”; 这种,那就是放在[常量池](https://so.csdn.net/so/search?q=常量池&spm=1001.2101.3001.7020)中.
...
![在这里插入图片描述](java基础/clip_image034.gif)
...
![深 拷 贝 和 浅 拷 贝  深 栳 贝 和 浅 拷 贝 孰 是 指 对 盅 的 拷 贝 ， 一个对象中存在蕊种类型的@性 ， 一 种 是 基 事 数 庭 类 型 ， 一 种 是 实 例 对 象 的 引 庠 ．  1 、 浅 拷 贝 是 挹 ， 只 会 拷 贝 基 本 數 类 型 匾 ， 以 及 实 树 象 引 的 地 址 。 荇 不 会 0 制 一 份 引 0 地 址 所 指 对 象 。 也 就 是 浅 拷 贝 出 来 对 象 。 丙 刁 类 性 尷 向 的 是 同 一 ^  对 象  2 ． 深 店 贝 是 ， 既 会 贝 基 本 数 廡 类 型 的 们 ， 也 会 针 对 买 例 对 的 引 厍 地 址 所 向 的 对 象 行 过 制 ， 深 满 贝 湖 来 的 对 象 ， 内 0 《 性 指 向 的 不 是 同 一 个 对 象 ](java基础/clip_image036.jpg)



## 泛型中extends和super的区别

![I. ? extends T  2. super ](java基础/clip_image038.jpg)