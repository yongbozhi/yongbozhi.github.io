...
![线 程 的 优 点 ：  。 一 个 进 程 中 可 以 同 时 存 在 个 线 程 ；  。 各 个 线 程 之 间 可 以 并 发 执 行 ；  。 各 个 线 程 之 间 可 以 共 享 地 址 空 间 和 文 件 等 资 源 ；  线 程 的 缺 点 ：  能 当 进 程 中 的 一 个 线 程 崩 溃 时 ， 会 导 致 其 所 属 进 程 的 所 有 线 程 崩 溃 。 ](并发/clip_image004.jpg)
...
![img](并发/clip_image008.jpg)

 

**实现**Callable接口

![img](并发/clip_image010.jpg)
...
![img](并发/clip_image018.jpg)![img](并发/clip_image020.jpg)
...
\3.  如果成功，当前线程获得轻量级锁，执行同步代码块。 如果失败，表示其他线程竞争锁，当前线程便尝试使用自旋来获取锁。当自旋次数达到一定次数时，锁就会升级为重量
...
![lock. lock();  try(  lock.unLock(); ](并发/clip_image060.jpg)
...
![* The synchronization state.  private volatile int state; ](并发/clip_image090.gif)

​    在多线程下，一定会有多个线程来同时修改state变量，所以在AQS中也提供了一些方法能够安全的对state值进行修改。分别为：

![* Returns the current value of synchronization state.  * This operation has memory semantics of a {@code volatile} read.  * @return current state value  protected final int getState() {  return state; ](并发/clip_image092.gif)

![* Sets the value of synchronization state.  * This operation has memory semantics of a {@code volatile} write.  * @param newState the new state value  protected final void setState(int newState) {  state  = newState; ](并发/clip_image094.gif)
...
![img](并发/clip_image139.jpg)

同时：

![img](并发/clip_image140.jpg)

依次：

![img](并发/clip_image141.jpg)

![img](并发/clip_image142.jpg)

![img](并发/clip_image143.jpg)

有序交错：

![img](并发/clip_image144.jpg)

![img](并发/clip_image145.jpg)

![img](并发/clip_image146.jpg)

![img](并发/clip_image147.jpg)
...
创建使用固定线程数的线程池。适用于为了满足资源管理而需要限制当前线程数量

的场景。同时也适用于负载较重的服务器。其定义如下：

![img](并发/clip_image164.jpg)
...
​    其是一个大小无界的线程池，会根据需要创建新线程。适用于执行很多的短期异步

任务的小程序或者是负载较轻的服务器。

corePoolSize 被设置为 0，即 corePool 为空；maximumPoolSize 被设置为Integer.MAX_VALUE。这里把 keepAliveTime 设置为 60L，意味着 CachedThreadPool中

的空闲线程等待新任务的最长时间为60秒，空闲线程超过60秒后将会被终止。
...
CachedThreadPool 会不断创建新线程。极端情况下，CachedThreadPool 会因为创建过
...
2.1  Future submit(Callable task);
...
![img](并发/clip_image170.jpg)

 

## 线程池中线程复用原理

![img](并发/clip_image172.jpg)

 

## 线程池调优

[**①** **线程池的调优（线程池的合理配置）**](https://mp.weixin.qq.com/s?__biz=MzUzMTA2NTU2Ng==&mid=2247487551&idx=1&sn=18f64ba49f3f0f9d8be9d1fdef8857d9&scene=21#wechat_redirect)
...
[**②** **线程池的监控**](https://mp.weixin.qq.com/s?__biz=MzUzMTA2NTU2Ng==&mid=2247487551&idx=1&sn=18f64ba49f3f0f9d8be9d1fdef8857d9&scene=21#wechat_redirect)
